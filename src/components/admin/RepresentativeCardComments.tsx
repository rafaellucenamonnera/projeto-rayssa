...
interface RepresentativeCardCommentsProps {
  cardId: string;
  currentStage: string;
  userName: string;
...
interface Comment {
  ...
  user_id: string;
  representative_card_comment_attachments?: Attachment[];
}
...
export const RepresentativeCardComments = ({
  cardId,
  currentStage,
  userName,
  ...
  canInsertFile = true,
}: RepresentativeCardCommentsProps) => {
  ...
  const loadComments = async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select(
        "*,representative_card_comment_attachments(id,storage_path,file_name,mime_type,size_bytes)"
      )
      ...
      if (staged.length > 0 && comment?.id) {
        for (const file of staged) {
          try {
            ...
            const path = `representative_cards/${cardId}/${comment.id}/${uid}.${extOf(file.name)}`;
            ...
            const { error: metaErr } = await (supabase as any)
              .from("representative_card_comment_attachments")
              .insert({
      ...
      {loading ? (
        ...
        <div className="space-y-3 max-h-60 overflow-y-auto">
          {comments.map((c) => {
            ...
            const attachments = c.representative_card_comment_attachments || [];
            return (
