...
WITH CHECK (length(nome_fantasia) > 0 AND length(cnpj) > 0 AND length(nome_responsavel) > 0 AND length(email_responsavel) > 0 AND parceiro_id IN (SELECT id FROM parceiros_comerciais WHERE ativo = true));
