...
BEGIN
  ...
  -- 1. Resolver embaixador pelo slug
  IF v_partner_slug IS NOT NULL THEN
    ...
    ORDER BY aprovado DESC NULLS LAST, created_at ASC NULLS LAST
    LIMIT 1;
  END IF;
  ...
  IF v_partner_id IS NULL THEN
    SELECT id INTO v_partner_id FROM public.parceiros_comerciais ORDER BY created_at ASC NULLS LAST LIMIT 1;
  END IF;
  ...
  -- 2. Match do lead: email -> telefone -> nome_fantasia (panel_id = 'comercial')
  ...
  -- 3. Upsert + sempre mover para Lead Qualificado
  IF v_lead_id IS NOT NULL THEN
    SELECT parceiro_id INTO v_lead_partner FROM public.leads WHERE id = v_lead_id;

    UPDATE public.leads SET
      panel_id                        = 'comercial',
      status_lead                     = 'etapa_comercial_1783879107510'::public.lead_status,
  ...
  ELSE
    INSERT INTO public.leads (
      ...
      'etapa_comercial_1783879107510'::public.lead_status, 'comercial', false,
  ...
  -- 4. Diagnóstico completo (uma linha por envio)
  ...
  -- 5. CTA reunião: reaproveitar tarefa pendente ou criar nova
  IF v_solicitou THEN
