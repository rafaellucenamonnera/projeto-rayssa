-- 1) Canva material generations: restringir a admin/gestor
...
CREATE POLICY "canva_material_generations_read"
  ON public.canva_material_generations FOR SELECT TO authenticated
...
CREATE POLICY "canva_material_generations_insert"
  ON public.canva_material_generations FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'gestor_conta'));

-- 2) submit_teste_monnera: lead_id do cliente deixa de ser prova de posse
...
BEGIN
  ...
  -- Alvo direto pelo lead_id: só é aceito se o lead realmente corresponder aos
  -- dados informados no formulário (e-mail, telefone ou empresa). O lead_id
  -- sozinho NÃO é prova de posse.
  IF v_incoming_lead IS NOT NULL THEN
    BEGIN
      ...
      WHERE id = v_incoming_lead::uuid
        ...
        AND (
          lower(coalesce(email_responsavel,'')) = v_email
          OR (length(v_tel_digits) >= 8
              AND regexp_replace(coalesce(telefone_responsavel,''), '\D', '', 'g') = v_tel_digits)
          OR lower(coalesce(nome_fantasia,'')) = lower(v_empresa)
        )
      LIMIT 1;
