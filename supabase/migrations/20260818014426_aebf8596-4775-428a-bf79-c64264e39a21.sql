...
BEGIN
  ...
  v_already := v_row.matched_card_id IS NOT DISTINCT FROM p_card_id;
  ...
  IF length(v_msg_cnpj) <> 14 AND length(v_card_cnpj) = 14 THEN
    ...
    v_inherited := array_append(v_inherited, 'cnpj'::text);
  ...
  -- O CNPJ confirmado passa a ter origem no card sempre que coincidir com o CNPJ do card vinculado.
  v_card_source := length(v_card_cnpj) = 14
    AND v_card_cnpj = regexp_replace(coalesce(v_overrides->>'cnpj', v_msg_cnpj, ''), '\D', '', 'g');
  ...
  IF coalesce(btrim(v_card.full_name), '') <> '' AND v_nome IS DISTINCT FROM btrim(v_card.full_name) THEN
    ...
    v_inherited := array_append(v_inherited, 'nome_parceiro'::text);
    IF NOT v_already THEN
      INSERT INTO public.gmail_triage_corrections
        (gmail_message_row_id, field, old_value, new_value, justification, origin, evidence, created_by)
  ...
  IF v_card_source THEN v_row.cnpj_source := 'card_vinculado'; END IF;
  ...
  UPDATE public.gmail_processed_messages
     SET manual_overrides = v_overrides,
         ...
         cnpj_source = CASE WHEN v_card_source THEN 'card_vinculado'
                            ELSE coalesce(v_calc->>'cnpj_source', cnpj_source) END,
         ...
         linked_by = coalesce(linked_by, auth.uid()),
         linked_card_snapshot = jsonb_build_object(
           ...
           'cnpj_source', CASE WHEN v_card_source THEN 'card_vinculado' ELSE v_calc->>'cnpj_source' END,
           'antes', v_before,
           'confirmado_em', now(),
         ...
         released_at = CASE WHEN jsonb_array_length(v_calc->'triage_pending') = 0 THEN coalesce(released_at, now()) ELSE released_at END,
         released_by = CASE WHEN jsonb_array_length(v_calc->'triage_pending') = 0 THEN coalesce(released_by, auth.uid()) ELSE released_by END,
         last_correction_at = now()
   WHERE id = p_row_id;

  RETURN jsonb_build_object(
    'linked', true,
    'ja_vinculado', v_already,
    'card_id', p_card_id,
    'card_nome', v_card.full_name,
    'card_etapa', v_card.stage_id,
    'cnpj', v_calc->>'cnpj',
